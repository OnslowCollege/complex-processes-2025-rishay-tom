...
-- /api/users (GET)
-- A list of users, will use the DB, returns a user object 
-- pub struct User {
--     pub username: String,
...
-- /api/deleteuser (POST)
-- deletes the user. takes a modifyelementdata
...
-- /api/createuser (POST)
-- creates a user
-- create the user. takes a modifyelementdata
  ...
  case decoded of
    ...
    Right (Wrapped kind data_) -> do
      ...
      liftIO $ atomically $ do
        state <- readTVar stateVar
        writeTVar stateVar state { messages = messages state ++ [dataMessage data_] } -- add new message(s)
...
usersHandler :: ActionM ()
usersHandler = do

 liftIO $ do
        modifyMVar_ stateVar $ \state -> return state 
          { users = users state ++ [dataUsers data_] }
