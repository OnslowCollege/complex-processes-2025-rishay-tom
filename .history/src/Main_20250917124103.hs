...
pracHandler stateVar broadcastChan = do
  ...
  case decoded of 
    ...
    Right (Wrapped kind data_) -> do
      ...
      -- Also add to HTTP polling for compatibility
      liftIO $ addBroadcastMessage stateVar ("Practice: " ++ dataMessage data_)

      liftIO $ putStrLn $ "Received: " ++ show (Wrapped kind data_)
      json jsonVal

-- HTTP Polling endpoint (for compatibility)
...
main = do
  ...
  -- Initialize shared state and broadcast channel
  stateVar <- newTVarIO (AppState 0 [] [] 1 [])
  broadcastChan <- newBroadcastTChanIO
  
  -- Fork the channel handler
  ...
  scottyApp <- scottyApp $ do
    ...
    get "/api/ws" (messagesHandler stateVar)  -- HTTP fallback
  ...
  putStrLn "WebSocket available at: ws://localhost:7879/"
  putStrLn "HTTP API available at: http://localhost:7879/api/"
