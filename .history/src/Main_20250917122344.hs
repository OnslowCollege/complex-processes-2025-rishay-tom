...
-- Modified handlers to work with MVar and broadcasting
...
mainHandler stateVar = do
  ...
  case decoded of
    ...
    Right (Wrapped kind data_) -> do
      ...
      -- Add message to state and prepare for broadcast
      liftIO $ do
        modifyMVar_ stateVar $ \state -> return state 
...
pracHandler stateVar = do
  ...
  case decoded of 
    ...
    Right (Wrapped kind data_) -> do
      ...
      -- Update state and prepare broadcast
      liftIO $ do
        modifyMVar_ stateVar $ \state -> return state 
          { messages = messages state ++ [dataMessage data_] }
...
-- New endpoint to poll for broadcast messages (alternative to WebSocket)
...
main = do
  ...
  -- Initialize shared state using MVar
