...
data AppState = AppState
  { userCount :: Int
  , messages  :: [String]
  , broadcastMessages :: [String]  -- Messages to broadcast
  } deriving (Show)

-- Channel message types (for future WebSocket support)
data ChannelMessage 
  = UserMessage String
...
websocketHandler :: TVar AppState -> TChan ChannelMessage -> ActionM ()
websocketHandler stateVar broadcastChan = do
  bodyText <- body
  liftIO $ putStrLn $ "Raw body received: " ++ show (BL.take 200 bodyText)
...
main = do
 ...
 scotty 7879 $ do
   ...
   post "/api/ws" (mainHandler stateVar broadcastChan)